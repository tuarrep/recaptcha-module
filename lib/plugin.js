...
class ReCaptcha {
  constructor ({ hideBadge, language, siteKey, version, size }) {
    console.warn("Building local fork ReCaptcha")
    ...
    this.randomName = Math.round(Math.random() * 10e16)
    this[this.randomName] = {};

    console.warn(`builded with randomName=${this.randomName}`)
  }
  ...
  async execute (action) {
    try {
      ...
      if (this.randomName in this) {
        return this[this.randomName].execute(
          this.siteKey,
          { action }
        )
      }
    ...
    return new Promise((resolve, reject) => {
      if (this.randomName in this) {
        if(this.size == 'invisible'){
          this[this.randomName].execute(widgetId)
          ...
          const response = this[this.randomName].getResponse(widgetId)
      ...
      script.addEventListener('load', () => {
        ...
        window.grecaptcha.ready(() => {
          if('grecaptcha' in window) {
            this[this.randomName] = {... window.grecaptcha}
            window.grecaptcha = {
              execute: () => 'FVCK 0F!',
              init: () => 'FVCK 0F!'
            }
  ...
  reset (widgetId) {
    if (this.version === 2 || typeof widgetId !== 'undefined') {
      this[this.randomName].reset(widgetId)
    }
  }

  render (reference, { sitekey, theme }) {
    return this[this.randomName].render(reference.$el || reference, { sitekey, theme })
  }
}
